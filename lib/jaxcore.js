...
class Jaxcore extends Service {
	...
	connectAdapter(device, adapterProfileName, callback) {
		...
		if (device.deviceType === deviceType) {
			...
			quit('device', device.id, 'is of the wrong type', device.deviceType, '!=', deviceType);
		}
	}
	
	createAdapter(device, adapterType, profile, callback) {
		if (!profile) profile = {};
		this.log('CREATING ADAPTER:', device.id, adapterType, profile);
	...
	launchAdapter(device, adapterType, config, callback) {
		const adapterConfig = this.findSpinAdapter(device); //todo: filter adapters by adapterType, config
		...
		if (adapterConfig) {
			...
			this.log('DID NOT FIND ADAPTER FOR:', device.id, config);
			...
			this.createAdapter(device, adapterType, config, (err, adapterInstance, adapterConfig) => {
				if (err) {
					...
					if (callback) callback(err, adapterInstance, adapterConfig, true);
				}
			});
		}
	}
